...
export default function AddPropertyPage() {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            await postWithAuth("/properties", {
                ...formData,
                price: parseFloat(formData.price),
                attributes: processedAttributes
            });
            webApp.showConfirm("Property listed successfully!");
            router.push("/merchant/dashboard");
        } catch (error: any) {
            webApp.showAlert(error.message || "Failed to create property.");
        } finally {
            setSubmitting(false);
        }
    };
    ...
    return (
        <div className="min-h-screen bg-zinc-50 pb-32 dark:bg-black">
            ...
            <div className="bg-white px-6 pt-12 pb-8 shadow-sm dark:bg-zinc-900">
                <h1 className="text-3xl font-black tracking-tight text-zinc-900 dark:text-white">New Listing</h1>
                <p className="mt-2 text-zinc-500 dark:text-zinc-400">Add a premium property to the market</p>
            </div>
